...
export function useFacilityMutations() {
  ...
  return {
    ...
    update: useMutation({ mutationFn: (input: { id: string; payload: Partial<FacilityPayload> }) => facilitiesApi.update(input.id, input.payload), onSuccess: (_data: Facility, variables: { id: string }) => refreshDetail(variables.id) }),
